...
import { Camera, FileText, Play, CheckCircle, RotateCcw, Moon, Sun, Edit3, Keyboard, RefreshCw, Eye, ArrowDown, BarChart2, Layers, History, Save, Download, Printer, Trash2, Palette, Cpu } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const loadFromHistory = (item: HistoryItem) => {
      ...
      // Auto solve on load? Optional. Let's let user click solve.
      setError(null);
  };
        ...
        {problem && (
            <div className="grid lg:grid-cols-2 gap-8 animate-in slide-in-from-bottom-8 duration-500 mb-20">
                ...
                <div className="space-y-6">
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 break-inside-avoid">
                         <div className="flex justify-between items-start mb-4">
                             <h3 className="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                                <Eye className="w-5 h-5 text-primary-500" /> Source Input
                             </h3>
                             <button onClick={handlePrintPDF} className="text-xs flex items-center gap-1 text-slate-500 hover:text-primary-600 transition no-print">
                                 <Printer className="w-3 h-3"/> Print PDF
                             </button>
                         </div>
                         <div className="bg-slate-100 dark:bg-slate-900 rounded-lg p-4 min-h-[100px] flex items-center justify-center border border-slate-200 dark:border-slate-800">
                            {inputMode === 'image' && imagePreview ? <img src={imagePreview} className="max-h-48 object-contain rounded"/> : <p className="text-sm font-mono text-slate-600 dark:text-slate-400 whitespace-pre-wrap">{inputMode === 'manual' ? 'Manual Entry' : textInput}</p>}
                         </div>
                    </div>
                ...
                <div className="space-y-6 break-inside-avoid">
                    {problem.variables.length === 2 ? (
                         <div className="bg-slate-50 dark:bg-slate-900/50 p-1.5 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700 h-fit shadow-sm">
            ...
            <div id="solution-start" className="max-w-7xl mx-auto border-t border-slate-200 dark:border-slate-800 pt-12 pb-32">
                ...
                <div className="space-y-12">
                    {steps.slice(0, visibleStepCount).map((step, idx) => (
                        <div key={idx} className="animate-in slide-in-from-bottom-8 fade-in duration-700 relative break-inside-avoid">
                             {/* Connector Line */}
