...
// Initialize Gemini on the server where process.env.API_KEY is available.
...
export async function extractLPFromImage(base64Image: string, mimeType: string): Promise<LPProblem> {
...
export async function extractLPFromText(textInput: string): Promise<LPProblem> {
    const modelId = "gemini-2.5-flash";
    const prompt = `
